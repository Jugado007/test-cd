...
const Binary = () => {
  ...
  const handleClick = () => {
    if (text === "") {
      clipboard(binary.convert("Type or paste your content here"));
      alert(`!You have copied`);
    } else {
      clipboard(binary.convert(text));
      alert(`!You have copied "${binary.convert(text)}"`);
    }
  };

  const onClickDownload = () => {
    if (text === "") {
      const data = binary.convert("Type or paste your content here");
      ...
      const element = document.createElement("a");
      const file = new Blob([binary.convert(text)], {
        type: "text/plain;charset=utf-8",
      });
      element.href = URL.createObjectURL(file);
      element.download = "convertcase-net.txt";
          ...
          <div className="">
            <div>
              ...
              <p className="text-sm font-normal">
                ...
                quickly convert your English text into a binary code. Simply
                enter the regular text as you would in the left column panel and
                then you will see it automatically get translated into a series
                of binary code on the right. Likewise, if you have binary code -
                enter it into the right panel of the binary translator and get
