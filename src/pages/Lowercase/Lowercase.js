...
const Mirrortext = () => {
  ...
  const handleClick = () => {
    if (text === "") {
      clipboard("Type or Paste Your Content Here".toLowerCase());
  ...
  const onClickDownload = () => {
    if (text === "") {
      const data = "Type or Paste Your Content Here".toLowerCase();
      ...
      const element = document.createElement("a");
      const file = new Blob([text.toLowerCase()], {
        type: "text/plain;charset=utf-8",
      });
      element.href = URL.createObjectURL(file);
      element.download = "convertcase-net.txt";
